...
static int check_arg(char** argv)  // Vai verificar se o arquivo tem a extensão .cub
{
    char **split;
...
{
    char *current_line;
    current_line = get_next_line(fd);  // Pega a próxima linha
...
static int get_elements(char *current_line, char *ident, t_file_info *file_info, int fd)
{
    int i;
...
int    get_files(t_cub3d *cub, char** argv)
{
    ...
    cub->map = create_map(fd, current_line); //vai pegar todas as informações vinda do mapa
    return status;
}

static unsigned int get_colors(int fd, char ***floor, char ***ceiling, char *current_line)
{
